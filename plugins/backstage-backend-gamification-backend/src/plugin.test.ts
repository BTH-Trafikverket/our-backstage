...
import { backstageBackendGamificationPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    ...
    await request(server)
      .get(`/api/backstage-backend-gamification/todos/${createdTodoItem.id}`)
  ...
  it('should create TODO item with catalog information', async () => {
    const { server } = await startTestBackend({
      features: [
        backstageBackendGamificationPlugin,
