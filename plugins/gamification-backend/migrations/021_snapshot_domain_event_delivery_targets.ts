...
export async function up(knex: Knex): Promise<void> {
  await knex.schema.alterTable('domain_events', table => {
    ...
    BEGIN
      ...
      IF (NEW.completion_count % v_target_count) = 0 THEN
        ...
        inserted_domain_event AS (
          ...
          SELECT
            ...
            COALESCE((
              ...
              FROM webhooks w
              WHERE w.trigger_event_name = 'quest.completed'
            ), '[]'::jsonb),
        ...
        SELECT id, event_name
          INTO v_domain_event_id, v_domain_event_name
        FROM inserted_domain_event
        LIMIT 1;

        IF v_domain_event_id IS NOT NULL THEN
          PERFORM notify_gamification_domain_event(
            v_domain_event_id,
            v_domain_event_name
          );
        END IF;
    ...
    BEGIN
      ...
      VALUES (
        ...
        COALESCE((
          ...
          FROM webhooks w
          WHERE w.trigger_event_name = 'badge.earned'
        ), '[]'::jsonb),
        NEW.earned_at,
        NEW.earned_at
      )
...
export async function down(knex: Knex): Promise<void> {
  await knex.schema.alterTable('domain_events', table => {
    table.dropColumn('delivery_targets');
  });
}
